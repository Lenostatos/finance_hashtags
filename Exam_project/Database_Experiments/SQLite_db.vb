...
Public Class SQLite_db
    ...
    Private m_db_adapter As SQLiteDataAdapter
    ...
    Public Sub New()
        ...
        Dim db_connection As New SQLiteConnection(cnn_string)
        ...
        Dim query_string As New SQLiteCommand("SELECT * FROM " + table_names(1), db_connection)

        m_db_adapter = New SQLiteDataAdapter(query_string.CommandText, db_connection)

        'I might need the command builder to use the update method of the dbAdapter
        Dim query_builder As New SQLiteCommandBuilder(m_db_adapter)
